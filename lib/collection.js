...
module.exports = function Collection(db, state) {
  ...
  var interface = {
    ...
    insert: function(docs, options, callback) {
      ...
      //make copies
      docs = docs.map(function (doc) {
        return _.cloneDeep(doc, cloneObjectIDs)
      });
    ...
    update: function (selector, data, options, callback) {
      ...
      debug('%s.%s %j', name, (options.upsert?"upsert: ":"update: "), selector);
      ...
      asyncish(function() {
        ...
        if(!docs.length && options.upsert) {
          var cloned = _.clone(data.$setOnInsert || data.$set || data, cloneObjectIDs);
...
function cloneObjectIDs(value) {
  return value instanceof ObjectId? ObjectId(value) : value;
}
