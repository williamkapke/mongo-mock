...
module.exports = function Collection(db, state) {
  ...
  var interface = {
    ...
    insert: function(docs, options, callback) {
      ...
      asyncish(function () {
        ...
        for (var i = 0; i < docs.length; i++) {
          ...
          i === 0 ? insertedIds[1] = doc._id : insertedIds.push(doc._id)
        }

        state.persist();
        callback(null, {
          insertedIds: insertedIds,
          insertedCount: insertedIds.length-1,
    ...
    insertOne: function(doc, options, callback) {
      debug('insertOne %j', doc);
      callback = arguments[arguments.length-1];
      //if(callback===options) options = {};//ignored when mocking
      if(typeof doc!=='object') throw new Error("Failed inserting: document must be an object")
      if(name==='system.indexes') return interface.createIndex(doc, callback)
      ...
      asyncish(function () {
        ...
        callback(null, {
          ...
          result: {ok:1,n:1},
          connection: {},
          ops: [_.cloneDeep(doc, cloneObjectIDs)]
        });
      });
      if(typeof callback!=='function') {
        return new Promise(function (resolve,reject) {
