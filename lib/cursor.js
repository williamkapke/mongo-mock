...
var Cursor = module.exports = function(documents, opts) {
  ...
  var interface = {
    ...
    _triggerStream: function (documents) {
      setTimeout(() => {
    ...
    on: function (event, fn) {
      switch (event) {
        case 'data': {
          interface.rewind();
          var documents = getDocs(true);
          this._triggerStream(documents);
          break;
        }
      }
...
Cursor.prototype._read = function _read(size) {
  var self = this;
  ...
  console.log(self);
  if (!self.resultSet.length && self.currentReadIndex > 0) self.resultSet = getDocs(true);
  // If we provided a transformation method
  var result = self.resultSet[self.currentReadIndex] || null;
  if (typeof self.streamOptions.transform == 'function' && result != null) {
    return self.push(self.streamOptions.transform(result));
  }
